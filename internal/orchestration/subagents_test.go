...
func TestProjectLocalNarrowOnly(t *testing.T) {
	// Widen tokenBudget beyond investigator (24000) -> reject.
	wide := strings.Replace(validSubagentMD, "tokenBudget: 24000", "tokenBudget: 999999", 1)
	...
	// Global layer is not subject to narrowing (same file parses as global).
	if _, err := ParseSubagentContent("explorer.md", wide, SubagentSourceGlobal); err != nil {
		t.Fatalf("global widen should be allowed: %v", err)
	}
}
