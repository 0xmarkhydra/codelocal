...
func Route(task string, caps Capabilities) Decision {
	...
	if codeIntent && (caps.LSP || caps.Filesystem) {
		fallbacks := []Lane{}
		if caps.Shell { fallbacks = append(fallbacks, LaneShell) }
		if browserIntent && caps.Browser { fallbacks = append(fallbacks, LaneBrowser) }
		if desktopIntent && caps.Computer { fallbacks = append(fallbacks, LaneComputer) }
	...
	if browserIntent && caps.Browser {
		fallbacks := []Lane{}
		if caps.Computer { fallbacks = append(fallbacks, LaneComputer) }
	...
	if caps.LSP || caps.Filesystem { return Decision{Primary: LaneCode, Reason: "default to structured local context"} }
	if caps.Shell { return Decision{Primary: LaneShell, Reason: "shell is the best available local surface"} }
	if caps.Browser { return Decision{Primary: LaneBrowser, Reason: "browser is the best available structured UI surface"} }
