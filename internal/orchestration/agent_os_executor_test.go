...
func (runnerAdapter) Capabilities()agentruntime.Capabilities{return agentruntime.Capabilities{NonInteractive:true,StructuredOutput:true,FileEditing:true,ShellExecution:true,MCP:true,PlanMode:true,ReviewMode:true,Transport:agentruntime.TransportNativeStructured,Isolation:agentruntime.IsolationMediated}}
func (a runnerAdapter) Probe(context.Context)agentruntime.ProbeResult{return agentruntime.ProbeResult{EngineID:a.ID(),DisplayName:a.DisplayName(),Installed:true,Compatible:true,Auth:agentruntime.AuthAuthenticated,Capabilities:a.Capabilities()}}
func (a runnerAdapter) Start(context.Context,agentruntime.Request)(agentruntime.Session,error){ch:=make(chan agentruntime.Event);close(ch);return &runnerSession{events:ch},nil}
...
func (*runnerSession) Wait(context.Context)(agentruntime.Result,error){return agentruntime.Result{SessionID:"session-1",EngineID:"runner-fake",ProviderStatus:"completed"},nil}
...
func TestExecutePreparedAgentOSCompletesFocusedTask(t *testing.T){
	registry:=agentruntime.NewRegistry(); if err:=registry.Register(runnerAdapter{});err!=nil{t.Fatal(err)}
	...
	if err!=nil{t.Fatal(err)}
	if !report.Completed || len(report.Agents)!=1{t.Fatalf("unexpected report: %+v",report)}
	if report.Usage.OpenReservations!=0{t.Fatalf("reservations must settle: %+v",report.Usage)}
}
