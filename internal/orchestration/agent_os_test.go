...
type coordinatorAdapter struct { id string; caps agentruntime.Capabilities }
func (a coordinatorAdapter) ID() string { return a.id }
func (a coordinatorAdapter) DisplayName() string { return a.id }
func (a coordinatorAdapter) Probe(context.Context) agentruntime.ProbeResult { return agentruntime.ProbeResult{EngineID:a.id,Installed:true,Compatible:true,Auth:agentruntime.AuthAuthenticated,Capabilities:a.caps} }
func (a coordinatorAdapter) Capabilities() agentruntime.Capabilities { return a.caps }
func (a coordinatorAdapter) Start(context.Context, agentruntime.Request) (agentruntime.Session,error) { return nil,nil }
...
func TestPrepareAgentOSKeepsFocusedTaskSingleAgent(t *testing.T) {
	...
	if err!=nil {t.Fatal(err)}
	if len(prepared.Agents)!=1 || len(prepared.Team.Members)!=1 || prepared.Team.Parallelism!=1 {t.Fatalf("focused task over-orchestrated: %+v",prepared.Team)}
	if prepared.Kernel.Snapshot().Stage!=KernelUnderstand {t.Fatalf("kernel not initialized: %+v",prepared.Kernel.Snapshot())}
	if len(prepared.Runtime.DAG().Nodes())!=1 {t.Fatalf("DAG not compiled: %+v",prepared.Runtime.DAG().Nodes())}
}
...
func TestPrepareAgentOSCompilesComplexTeamIntoDurableGraphDAG(t *testing.T) {
	...
	if err!=nil {t.Fatal(err)}
	if prepared.Team.Tier!=TaskTier4 || len(prepared.Agents)<5 {t.Fatalf("complex plan missing team: %+v",prepared.Team)}
	if len(prepared.Runtime.Graph().Descendants(prepared.LeadAgentID))!=len(prepared.Agents)-1 {t.Fatalf("graph topology mismatch: agents=%d descendants=%d",len(prepared.Agents),len(prepared.Runtime.Graph().Descendants(prepared.LeadAgentID)))}
	if len(prepared.Runtime.DAG().Nodes())!=len(prepared.Agents) {t.Fatalf("DAG/member mismatch")}
	if prepared.Usage.Snapshot().OpenReservations!=len(prepared.Team.Members) {t.Fatalf("budget admission missing: %+v",prepared.Usage.Snapshot())}
}
...
func TestPrepareAgentOSFailsBeforeDispatchWhenNoSafeEngine(t *testing.T) {
	...
	if err==nil {t.Fatal("unsafe engine should not be admitted")}
}
