...
func ExecutePreparedAgentOS(ctx context.Context, prepared *PreparedAgentOS, engines *agentruntime.Runtime, req AgentOSExecutionRequest) (AgentOSExecutionReport, error) {
	if prepared == nil || prepared.Runtime == nil || engines == nil || engines.Registry == nil {
		return AgentOSExecutionReport{}, ErrAgentOSExecutionBlocked
	}
	req.WorkspaceKey = strings.TrimSpace(req.WorkspaceKey)
	req.TaskID = strings.TrimSpace(req.TaskID)
	req.Objective = strings.Join(strings.Fields(req.Objective), " ")
	if req.WorkspaceKey == "" || req.TaskID == "" || req.Objective == "" {
		return AgentOSExecutionReport{}, ErrAgentOSExecutionBlocked
	}
	if req.RuntimeProvider == "" {
		req.RuntimeProvider = "engine"
	}
	parallel := req.MaxParallel
	if parallel <= 0 {
		parallel = prepared.Team.Parallelism
	}
	if parallel <= 0 {
		parallel = 1
	}
	if prepared.Team.Parallelism > 0 && parallel > prepared.Team.Parallelism {
		parallel = prepared.Team.Parallelism
	}

	report := AgentOSExecutionReport{Agents: []AgentExecutionResult{}}
	byNode := map[string]PlannedAgent{}
	for _, planned := range prepared.Agents {
		byNode[planned.NodeID] = planned
	}

	for {
		runnable := prepared.Runtime.DAG().Runnable()
		if len(runnable) == 0 {
			if allTaskNodesCompleted(prepared.Runtime.DAG().Nodes()) {
				break
			}
			return finalizeExecutionReport(prepared, report), ErrAgentOSExecutionBlocked
		}
		if len(runnable) > parallel {
		...
		for item := range outcomes {
			if item.result.AgentID != "" {
				report.Agents = append(report.Agents, item.result)
			}
		...
		if firstErr != nil {
			return finalizeExecutionReport(prepared, report), firstErr
		}
	}
	...
	if req.Verify != nil {
		...
		if err != nil {
			return finalizeExecutionReport(prepared, report), err
		}
		for _, item := range evidence {
			snapshot := prepared.Runtime.Verification().Snapshot()
			if _, err := prepared.Runtime.Verification().Record(snapshot.Revision, item); err != nil {
				return finalizeExecutionReport(prepared, report), err
			}
		}
	}
	if !prepared.Runtime.Verification().CanComplete() {
		return finalizeExecutionReport(prepared, report), ErrAgentOSVerification
	}
	lead, ok := prepared.Runtime.Graph().Agent(prepared.LeadAgentID)
	if !ok {
		return finalizeExecutionReport(prepared, report), ErrAgentOSExecutionBlocked
	}
	if _, err := prepared.Runtime.FinalizeLead(lead.ID, lead.Revision, time.Now().UTC()); err != nil {
		return finalizeExecutionReport(prepared, report), err
	}
...
func executePreparedNode(ctx context.Context, prepared *PreparedAgentOS, engines *agentruntime.Runtime, req AgentOSExecutionRequest, node TaskNode, planned PlannedAgent) (AgentExecutionResult, error) {
	...
	if !ok {
		return AgentExecutionResult{}, ErrAgentOSExecutionBlocked
	}
	...
	if identity.Status != agentruntime.AgentActive {
		return AgentExecutionResult{}, ErrAgentOSExecutionBlocked
	}
	...
	if _, err := prepared.Runtime.StartNode(node.ID, node.Revision, identity.ID); err != nil {
		return AgentExecutionResult{}, err
	}

	mode := agentruntime.ModeMutate
	if planned.Member.ReadOnly || planned.Member.VerificationOnly {
	...
	if current, ok := prepared.Runtime.Activations().Activation(activation.ID); ok {
		_, _ = prepared.Runtime.Activations().TransitionCAS(current.ID, current.Revision, agentruntime.ActivationCompleted, time.Now().UTC())
	}
	...
	if currentAgent.Status != agentruntime.AgentCompleted {
		if _, err := prepared.Runtime.Graph().TransitionCAS(currentAgent.ID, currentAgent.Revision, agentruntime.AgentCompleted, time.Now().UTC()); err != nil {
			return AgentExecutionResult{}, err
		}
	}
	...
	if _, err := prepared.Runtime.CompleteNode(freshNode.ID, freshNode.Revision, identity.ID); err != nil {
		return AgentExecutionResult{}, err
	}
	settleReservation(prepared, planned.Member.ID, req.UsageFromResult, result)
...
func markPreparedFailure(prepared *PreparedAgentOS, nodeID, agentID, activationID string) {
	if activation, ok := prepared.Runtime.Activations().Activation(activationID); ok {
		_, _ = prepared.Runtime.Activations().TransitionCAS(activation.ID, activation.Revision, agentruntime.ActivationFailed, time.Now().UTC())
	}
	if agent, ok := prepared.Runtime.Graph().Agent(agentID); ok && agent.Status != agentruntime.AgentFailed {
		_, _ = prepared.Runtime.Graph().TransitionCAS(agent.ID, agent.Revision, agentruntime.AgentFailed, time.Now().UTC())
	}
	if node, ok := prepared.Runtime.DAG().Node(nodeID); ok && node.Status != TaskNodeFailed {
		_, _ = prepared.Runtime.DAG().SetStatus(node.ID, node.Revision, TaskNodeFailed)
	}
}
...
func reservationFor(prepared *PreparedAgentOS, memberID string) (usage.Reservation, bool) {
	...
	for _, reservation := range prepared.Reservations {
		if strings.HasSuffix(reservation.ID, needle) {
			return reservation, true
		}
	}
	return usage.Reservation{}, false
}
func releaseReservation(prepared *PreparedAgentOS, memberID string) {
	if reservation, ok := reservationFor(prepared, memberID); ok {
		_, _ = prepared.Usage.Release(reservation.ID)
	}
}
func settleReservation(prepared *PreparedAgentOS, memberID string, extractor UsageExtractor, result agentruntime.Result) {
	reservation, ok := reservationFor(prepared, memberID)
	if !ok {
		return
	}
	...
	if extractor != nil {
		...
		if strings.TrimSpace(actual.TaskID) == "" {
			actual.TaskID = reservation.Usage.TaskID
		}
		if strings.TrimSpace(actual.AgentID) == "" {
			actual.AgentID = reservation.Usage.AgentID
		}
	}
	_, _, _ = prepared.Usage.Settle(reservation.ID, actual)
}
func allTaskNodesCompleted(nodes []TaskNode) bool {
	if len(nodes) == 0 {
		return false
	}
...
func finalizeExecutionReport(prepared *PreparedAgentOS, report AgentOSExecutionReport) AgentOSExecutionReport {
	if prepared == nil {
		return report
	}
	report.Verification = prepared.Runtime.Verification().Snapshot()
	report.Usage = prepared.Usage.Snapshot()
	return report
}
