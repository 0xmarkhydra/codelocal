...
func PrepareAgentOS(ctx context.Context, events *runtimeevents.Store, registry *agentruntime.Registry, request AgentOSRequest) (*PreparedAgentOS, error) {
	...
	releaseReservations := func() { for _, reservation := range reservations { _, _ = ledger.Release(reservation.ID) } }

	probes := registry.ProbeAll(ctx)
	planned := make([]PlannedAgent, 0, len(team.Members))
	for _, member := range team.Members {
		mode := agentruntime.ModeMutate
		if member.ReadOnly || member.VerificationOnly { mode = agentruntime.ModeReview }
	...
	if err != nil { releaseReservations(); return nil, err }
	leadPlan := planned[0]
	...
	for index := range planned {
		...
		if index > 0 {
			...
			if spawnErr != nil { releaseReservations(); return nil, spawnErr }
			item.AgentID = child.ID
		}
		blocked := make([]string, 0, len(item.Member.DependsOn))
		for _, dep := range item.Member.DependsOn { blocked = append(blocked, "node:"+dep) }
		...
		if addErr != nil { releaseReservations(); return nil, addErr }
		item.NodeID = node.ID
	}
...
func specialistAgentRole(role Specialist) agentruntime.AgentRole {
	switch role {
	case SpecialistInvestigator: return agentruntime.RoleInvestigator
	case SpecialistReviewer, SpecialistSecurity: return agentruntime.RoleReviewer
	case SpecialistTester: return agentruntime.RoleTester
	case SpecialistImplementer: return agentruntime.RoleImplementer
	default: return agentruntime.RoleSpecialist
	}
}
