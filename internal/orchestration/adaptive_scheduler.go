...
func ClassifyTaskTier(shape TaskShape) TaskTier {
	...
	if shape.Architectural || shape.Subsystems >= 4 || shape.FilesEstimated >= 12 {
		return TaskTier4
	}
	if shape.SecuritySensitive || shape.Subsystems >= 3 || shape.FilesEstimated >= 7 || strings.Contains(kind, "migration") {
		return TaskTier3
	}
	if shape.Subsystems >= 2 || shape.FilesEstimated >= 3 || strings.Contains(kind, "refactor") {
		return TaskTier2
	}
	if shape.FilesEstimated <= 1 && !shape.NeedsProductVerification && !strings.Contains(kind, "implement") {
		return TaskTier0
	}
	return TaskTier1
}
...
func PlanAdaptiveTeam(shape TaskShape, policies map[Specialist]SpecialistPolicy) TeamPlan {
	...
	case TaskTier3:
		if shape.SecuritySensitive {
			add("security", SpecialistSecurity)
		} else {
	...
	case TaskTier4:
		...
		if shape.SecuritySensitive {
			add("security", SpecialistSecurity, "implement")
		}
	}
	if shape.NeedsProductVerification {
		present := false
		for i := range members {
			if members[i].Role == SpecialistTester {
				members[i].VerificationOnly = true
				present = true
			}
		}
...
func ReserveTeamBudget(ledger *usage.Ledger, taskID string, plan TeamPlan) ([]usage.Reservation, error) {
	if ledger == nil {
		return nil, ErrTeamBudgetDenied
	}
