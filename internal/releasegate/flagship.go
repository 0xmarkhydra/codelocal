...
func EvaluateFlagship(e FlagshipEvidence) FlagshipResult {
	...
	for _,check:=range checks { if !check.ok { missing=append(missing,check.name) } }
...
func RequiredChaosScenarios() []ChaosScenario { return []ChaosScenario{ChaosRuntimeCrash,ChaosProviderLoss,ChaosNetworkLoss,ChaosStalePatch,ChaosDirtyCheckout,ChaosActivationLoss,ChaosDuplicateDelivery,ChaosCorruptTail,ChaosLeaseDoubleSpend} }
func ChaosCoverage(results []ChaosResult) []string { seen:=map[string]bool{}; for _,r:=range results { if r.Passed {seen[r.ID]=true} }; missing:=[]string{}; for _,id:=range RequiredChaosScenarios(){ if !seen[string(id)] {missing=append(missing,string(id))} }; sort.Strings(missing); return missing }
