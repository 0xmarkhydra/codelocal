...
func NewChaosHarness(handlers map[ChaosScenario]ChaosHandler) *ChaosHarness {
	...
	for scenario, handler := range handlers {
		if handler != nil {
			copyHandlers[scenario] = handler
		}
	}
...
func (h *ChaosHarness) RunRequired(parent context.Context, perScenarioTimeout time.Duration) []ChaosResult {
	if perScenarioTimeout <= 0 {
		perScenarioTimeout = 30 * time.Second
	}
	if perScenarioTimeout > 5*time.Minute {
		perScenarioTimeout = 5 * time.Minute
	}
	results := make([]ChaosResult, 0, len(RequiredChaosScenarios()))
	for _, scenario := range RequiredChaosScenarios() {
		handler := ChaosHandler(nil)
		if h != nil {
			handler = h.handlers[scenario]
		}
		if handler == nil {
			results = append(results, ChaosResult{ID: string(scenario), Passed: false, Evidence: "handler_missing"})
			continue
		}
		ctx, cancel := context.WithTimeout(parent, perScenarioTimeout)
		evidence, err := handler(ctx)
		cancel()
		if err != nil {
			results = append(results, ChaosResult{ID: string(scenario), Passed: false, Evidence: "error:" + chaosErrorSignature(err)})
			continue
		}
...
func chaosErrorSignature(err error) string {
	if err == nil {
		return ""
	}
