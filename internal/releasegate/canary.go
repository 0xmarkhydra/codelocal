...
func DecideCanary(currentPercent int, gate Gate, flagship FlagshipResult) CanaryDecision {
	...
	if !gate.Passed || !flagship.Passed {
		return CanaryDecision{Action: RolloutRollback, FromPercent: currentPercent, ToPercent: 0, Reason: rolloutReason(gate, flagship)}
	}
	next := currentPercent
	switch {
	case currentPercent < 1:
		next = 1
	case currentPercent < 10:
		next = 10
	...
	if next == currentPercent {
		return CanaryDecision{Action: RolloutHold, FromPercent: currentPercent, ToPercent: currentPercent, Reason: "release_gates_stable"}
	}
	return CanaryDecision{Action: RolloutPromote, FromPercent: currentPercent, ToPercent: next, Reason: "release_gates_passed"}
}
func rolloutReason(gate Gate, flagship FlagshipResult) string {
	if !gate.Passed && len(gate.Blockers) > 0 {
		return "gate:" + strings.Join(gate.Blockers, ",")
	}
	if !flagship.Passed && len(flagship.Missing) > 0 {
		return "flagship:" + strings.Join(flagship.Missing, ",")
	}
