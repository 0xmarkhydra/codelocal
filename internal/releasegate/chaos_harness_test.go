...
func TestChaosHarnessRequiresFullCoverageAndRedactsErrors(t *testing.T) {
	...
	for _, scenario := range RequiredChaosScenarios() {
		s := scenario
		handlers[s] = func(context.Context) (string, error) { return string(s) + ":ok", nil }
	}
	...
	if len(results) != len(RequiredChaosScenarios()) {
		t.Fatalf("coverage=%d", len(results))
	}
	for _, result := range results {
		if result.ID == string(ChaosNetworkLoss) {
			if result.Passed {
				t.Fatal("expected network loss failure")
			}
			if result.Evidence == "" || result.Evidence == "Bearer super-secret-token" {
				t.Fatalf("raw error leaked: %q", result.Evidence)
			}
		}
	}
}
