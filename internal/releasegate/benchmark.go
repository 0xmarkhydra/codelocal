...
func AggregateSamples(samples []TaskSample) (Metrics, error) {
	if len(samples) == 0 {
		return Metrics{}, ErrNoBenchmarkSamples
	}
	verified := 0
	...
	for _, sample := range samples {
		if sample.Verified {
			verified++
			if sample.Tokens > 0 {
				verifiedTokens = append(verifiedTokens, sample.Tokens)
			}
		}
		...
		if sample.ResumeAttempted {
			resumeAttempts++
			if sample.ResumeSucceeded {
				resumeSuccess++
			}
		}
	...
	if resumeAttempts > 0 {
		metrics.ResumeSuccessRate = float64(resumeSuccess) / float64(resumeAttempts)
	}
...
func EvaluateSamples(baselineSamples, candidateSamples []TaskSample, infra Infrastructure, chaos []ChaosResult, cfg Config) (BenchmarkReport, error) {
	baseline, err := AggregateSamples(baselineSamples)
	if err != nil {
		return BenchmarkReport{}, err
	}
	candidate, err := AggregateSamples(candidateSamples)
	if err != nil {
		return BenchmarkReport{}, err
	}
...
func percentileInt64(values []int64, p float64) int64 {
	...
	index := int(float64(len(copyValues)-1)*p + .5)
	if index < 0 {
		index = 0
	}
...
func nonNegativeSample(value int64) int64 {
	if value < 0 {
		return 0
	}
	return value
