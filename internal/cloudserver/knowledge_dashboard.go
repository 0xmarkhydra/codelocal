...
func (s *Server) knowledgeDashboard(w http.ResponseWriter, r *http.Request, identity *webauth.Identity) {
	...
	if err != nil {
		http.Error(w, "Knowledge Graph is temporarily unavailable.", http.StatusInternalServerError)
		return
	}
	raw, _ := json.Marshal(graph)
	body := dashboardNav(identity) + fmt.Sprintf(`
<style>
...
.knowledge-toolbar{display:flex;gap:10px;align-items:center;flex-wrap:wrap;padding:12px;border-bottom:1px solid var(--border)}.knowledge-search,.knowledge-filter{height:38px;border:1px solid var(--border);border-radius:10px;background:var(--surface);color:var(--text);padding:0 12px}.knowledge-search{flex:1;min-width:220px}.knowledge-filter{min-width:150px}.knowledge-shell{display:grid;grid-template-columns:minmax(0,1fr) 290px;min-height:610px}.knowledge-canvas-wrap{position:relative;min-height:610px;overflow:hidden;background:radial-gradient(circle at 50%% 45%%,color-mix(in srgb,var(--surface) 92%%,transparent),var(--bg))}.knowledge-canvas{display:block;width:100%%;height:610px;cursor:grab}.knowledge-canvas.dragging{cursor:grabbing}.knowledge-help{position:absolute;left:14px;bottom:12px;padding:7px 10px;border:1px solid var(--border);border-radius:9px;background:color-mix(in srgb,var(--surface) 90%%,transparent);font-size:11px;color:var(--muted);pointer-events:none}.knowledge-detail{border-left:1px solid var(--border);padding:18px;overflow:auto}.knowledge-detail-empty{color:var(--muted);font-size:13px;line-height:1.6}.knowledge-kind{display:inline-flex;padding:4px 8px;border-radius:999px;background:var(--surface-2);font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);margin-bottom:10px}.knowledge-name{font-size:18px;font-weight:700;word-break:break-word}.knowledge-summary{font-size:13px;color:var(--muted);line-height:1.55;margin-top:10px;white-space:pre-wrap;word-break:break-word}.knowledge-meta{margin-top:16px;display:grid;gap:8px;font-size:12px}.knowledge-meta-row{display:flex;justify-content:space-between;gap:12px}.knowledge-meta-row span:first-child{color:var(--muted)}.knowledge-legend{display:flex;gap:12px;align-items:center;flex-wrap:wrap;font-size:11px;color:var(--muted)}.knowledge-dot{width:8px;height:8px;border-radius:50%%;display:inline-block;margin-right:5px}
@media(max-width:900px){.knowledge-stats{grid-template-columns:repeat(2,minmax(0,1fr))}.knowledge-shell{grid-template-columns:1fr}.knowledge-detail{border-left:0;border-top:1px solid var(--border);min-height:180px}.knowledge-canvas,.knowledge-canvas-wrap{min-height:520px;height:520px}}
</style>
...
<div class="card" style="overflow:hidden">
 <div class="knowledge-toolbar">
  ...
  <select id="knowledge-filter" class="knowledge-filter"><option value="all">All knowledge</option><option value="project">Projects</option><option value="repository">Repositories</option><option value="workspace">Workspaces</option><option value="memory">Memories</option><option value="device">Devices</option></select>
 ...
 <div class="knowledge-shell">
  <div class="knowledge-canvas-wrap"><canvas id="knowledge-canvas" class="knowledge-canvas"></canvas><div class="knowledge-help">Drag nodes · drag background to pan · wheel/pinch to zoom · click for details</div></div>
...
(()=>{
 ...
 const canvas=document.getElementById('knowledge-canvas'),wrap=canvas.parentElement,detail=document.getElementById('knowledge-detail'),search=document.getElementById('knowledge-search'),filter=document.getElementById('knowledge-filter');
 const ctx=canvas.getContext('2d'); const nodes=(payload.nodes||[]).map((n,i)=>({...n,vx:0,vy:0,x:0,y:0,index:i})); const edges=payload.edges||[]; const byId=new Map(nodes.map(n=>[n.id,n]));
 ...
 const memoryKinds=new Set(['event','artifact','goal','decision','preference','person','company','idea','constraint','milestone','problem','skill','memory']);
 ...
 let scale=1,panX=0,panY=0,selected=null,dragNode=null,panning=false,lastX=0,lastY=0,moved=false,query='',kindFilter='all',sim=0;
 ...
 canvas.addEventListener('mousedown',e=>{lastX=e.offsetX;lastY=e.offsetY;moved=false;dragNode=hit(lastX,lastY);panning=!dragNode;canvas.classList.add('dragging')});window.addEventListener('mousemove',e=>{if(!dragNode&&!panning)return;const r=canvas.getBoundingClientRect(),x=e.clientX-r.left,y=e.clientY-r.top,dx=x-lastX,dy=y-lastY;if(Math.abs(dx)+Math.abs(dy)>2)moved=true;if(dragNode){const p=world(x,y);dragNode.x=p.x;dragNode.y=p.y;dragNode.vx=dragNode.vy=0}else{panX+=dx;panY+=dy}lastX=x;lastY=y;draw()});window.addEventListener('mouseup',()=>{if(dragNode&&!moved)show(dragNode);dragNode=null;panning=false;canvas.classList.remove('dragging')});canvas.addEventListener('wheel',e=>{e.preventDefault();const before=world(e.offsetX,e.offsetY),factor=Math.exp(-e.deltaY*.001);scale=Math.max(.25,Math.min(3.2,scale*factor));const after=world(e.offsetX,e.offsetY);panX+=(after.x-before.x)*scale;panY+=(after.y-before.y)*scale;draw()},{passive:false});
 ...
 init();window.addEventListener('resize',resize);resize();requestAnimationFrame(physics);
	...
	_, _ = w.Write([]byte(ui.Page("CodeLocal Cloud · Knowledge Graph", "A living map of the projects, repositories and durable knowledge CodeLocal has learned for your account.", body)))
}
