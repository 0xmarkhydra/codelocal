...
func privacySections() []publicPageSection {
	return []publicPageSection{
		...
		{"Third parties and model providers", `<p>CodeLocal itself does not require a separate OpenAI API key for the standard ChatGPT plugin workflow. Content sent to ChatGPT or another model provider is governed by that provider and the user's selected product/workspace settings. Locally installed MCP extensions and websites opened through browser automation are separate third parties with their own policies.</p>`},
	}
}
...
func supportSections() []publicPageSection {
	return []publicPageSection{
		...
		{"First checks", `<ul><li>Run <code>codelocal --version</code> and confirm the expected release channel.</li><li>Run <code>codelocal</code> and confirm the paired runtime is online.</li><li>Run <code>codelocal .</code> only inside a project you intend to authorize.</li><li>If ChatGPT shows an old tool surface, reconnect the CodeLocal plugin after updating the runtime.</li></ul>`},
