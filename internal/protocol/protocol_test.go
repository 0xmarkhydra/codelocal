...
func TestCompatibilityAndSideEffects(t *testing.T) {
	...
	for _, tool := range []string{"write_file", "git_commit", "run_command", "mcp_call", "exec_write", "pty_write", "pty_resize", "exec_signal", "pty_signal", "exec_kill", "pty_kill", "browser_click", "computer_type"} {
