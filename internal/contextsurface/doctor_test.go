...
func TestDiagnoseFindsDuplicateAndPressure(t *testing.T) {
	...
	for _, finding := range report.Findings {
		foundDuplicate = foundDuplicate || finding.Kind == FindingDuplicateText
		foundPressure = foundPressure || finding.Kind == FindingBudgetPressure
	}
	if !foundDuplicate || !foundPressure {
		t.Fatalf("expected duplicate and pressure findings: %+v", report)
	}
}
...
func TestDiagnoseFailsHealthForUntrustedRequired(t *testing.T) {
	...
	report := Diagnose(surface)
	if report.Healthy {
		t.Fatalf("untrusted required context must not be healthy: %+v", report)
	}
}
