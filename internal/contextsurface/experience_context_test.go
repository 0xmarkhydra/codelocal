...
func TestCompileWithExperiencesIncludesOnlyPromotedCompatibleKnowledge(t *testing.T) {
	...
	for _, item := range surface.Items {
		if item.ID == "experience:good" {
			found = true
			if item.Trust != "verified" {
				t.Fatalf("experience trust lost: %+v", item)
			}
		}
		if item.ID == "experience:draft" || item.ID == "experience:other" {
			t.Fatalf("incompatible experience leaked into context: %+v", item)
		}
	}
	if !found {
		t.Fatal("expected promoted main-branch experience")
	}
}
