...
func integer(description string, min, max int) map[string]any {
	...
	if min != 0 { out["minimum"] = min }
	if max != 0 { out["maximum"] = max }
	return out
}
func array(items any, description string) map[string]any { return map[string]any{"type": "array", "items": items, "description": description} }
func anyObject(description string) map[string]any { return map[string]any{"type": "object", "additionalProperties": true, "description": description} }
...
func toolDisplayTitle(name, fallback string) string {
	titles := map[string]string{
		"list_devices": "Show active CodeLocal devices", "list_device_identities": "Show paired devices", "revoke_device": "Revoke paired device", "rename_device": "Rename paired device", "list_workspaces": "Show authorized workspaces", "select_workspace": "Choose workspace", "workspace_info": "Show current workspace", "project_info": "Inspect workspace", "project_map": "Scan project structure", "context_for_task": "Find relevant code", "read_instructions": "Read project instructions", "list_files": "Browse project files", "file_info": "Inspect file metadata", "read_file": "Read file", "read_file_range": "Read file section", "read_files": "Read selected files", "search_code": "Search project text", "inspect_dependency": "Inspect dependency", "read_dependency": "Read dependency source", "search_dependency": "Search dependency source", "semantic_info": "Inspect code intelligence", "workspace_symbols": "Find project symbols", "find_symbol": "Find symbol", "document_symbols": "Find file symbols", "find_definition": "Find definition", "find_references": "Find references", "find_implementations": "Find implementations", "get_hover": "Inspect symbol type", "get_diagnostics": "Check code diagnostics", "get_callers": "Find callers", "get_callees": "Find callees", "get_import_graph": "Inspect import graph", "write_file": "Write file", "edit_file": "Edit file", "apply_patch": "Apply code patch", "apply_edits": "Apply structured edits", "format_changed_files": "Format changed files", "snapshot_diagnostics": "Snapshot diagnostics", "verify_changes": "Verify code changes", "git_status": "Check Git status", "git_diff": "Review code changes", "git_log": "Read Git history", "git_show": "Inspect Git commit", "git_blame": "Inspect Git blame", "git_file_history": "Read file history", "git_stage": "Stage Git changes", "git_unstage": "Unstage Git changes", "git_commit": "Commit Git changes", "git_push": "Push Git commits", "sandbox_info": "Inspect execution security", "sandbox_smoke_test": "Check execution security", "terminal_preflight": "Check command safety", "terminal_history": "Read terminal history", "run_command": "Run command", "exec_start": "Start background command", "pty_start": "Start interactive command", "exec_poll": "Read command output", "pty_poll": "Read interactive output", "process_poll": "Read process output", "exec_write": "Write process input", "pty_write": "Write interactive input", "process_write": "Write process input", "pty_resize": "Resize interactive terminal", "exec_signal": "Signal process", "pty_signal": "Signal interactive process", "exec_kill": "Stop process", "pty_kill": "Stop interactive process", "process_kill": "Stop process", "exec_cancel": "Cancel command", "process_list": "Show running commands", "approval_list": "Show remembered approvals", "approval_revoke": "Forget approval", "approval_reset": "Reset approvals", "mcp_list": "Show installed MCP extensions", "mcp_search_tools": "Find MCP extension tools", "mcp_tool_info": "Inspect MCP extension tool", "mcp_call": "Call MCP extension tool",
	}
	if title := titles[name]; title != "" { return title }
	return fallback
}

func terminalExecutionTool(name string) bool {
	switch name { case "run_command", "exec_start", "pty_start": return true; default: return false }
}

func toolMutatesState(name string) bool {
	if protocol.SideEffecting(name) { return true }
	switch name { case "select_workspace", "revoke_device", "rename_device", "exec_write", "pty_write", "exec_signal", "pty_signal", "exec_kill", "pty_kill", "process_kill": return true; default: return false }
}
...
func toolAnnotations(def toolDef) *mcp.ToolAnnotations {
	...
	idempotent := false
	switch name {
	case "write_file", "edit_file", "apply_patch", "apply_edits", "format_changed_files", "run_command", "exec_start", "pty_start", "revoke_device", "approval_revoke", "approval_reset", "mcp_call", "browser_click", "browser_fill", "browser_press", "computer_focus", "computer_click", "computer_type", "computer_key", "computer_scroll", "computer_drag": destructive = true
	}
	switch name { case "run_command", "exec_start", "pty_start", "git_push", "mcp_call", "browser_open": openWorld = true }
	switch name { case "select_workspace", "write_file", "git_stage", "git_unstage", "approval_reset", "revoke_device": idempotent = true }
	return &mcp.ToolAnnotations{Title: toolDisplayTitle(name, def.Title), ReadOnlyHint: readOnly, DestructiveHint: boolPtr(destructive), IdempotentHint: idempotent, OpenWorldHint: boolPtr(openWorld)}
}

func legacyProtocolOneTool(name string) bool {
	switch name {
	case "project_info", "read_instructions", "list_files", "file_info", "read_file", "read_file_range", "read_files", "search_code", "inspect_dependency", "read_dependency", "search_dependency", "find_symbol", "find_definition", "find_references", "get_callers", "get_callees", "get_import_graph", "get_diagnostics", "write_file", "edit_file", "apply_patch", "git_status", "git_diff", "git_log", "git_show", "git_blame", "git_file_history", "run_command", "process_poll", "process_list", "process_write", "process_kill": return true
	default: return false
	}
}

func capabilityBool(capabilities map[string]any, name string) bool { value, _ := capabilities[name].(bool); return value }

func toolCapability(name string) string {
	if strings.HasPrefix(name, "git_") { return "git" }
	switch name {
	case "run_command", "exec_start", "exec_poll", "exec_write", "exec_signal", "exec_kill", "exec_cancel", "process_poll", "process_list", "process_write", "process_kill", "terminal_preflight": return "shell"
	case "terminal_history": return "terminalHistory"
	case "pty_start", "pty_poll", "pty_write", "pty_resize", "pty_signal", "pty_kill": return "pty"
	case "mcp_list", "mcp_search_tools", "mcp_tool_info", "mcp_call": return "mcpHub"
	case "approval_list", "approval_revoke", "approval_reset": return "approvalMemory"
	default: return "filesystem"
	}
}

func ensureToolSupported(def toolDef, workspace *gateway.WorkspaceView) error {
	if workspace == nil { return errors.New("workspace unavailable") }
	if automationTool(def.Name) { return ensureAutomationToolSupported(def, workspace) }
	if workspace.ProtocolVersion <= 1 {
		if !legacyProtocolOneTool(def.Name) { return fmt.Errorf("%s requires a newer CodeLocal client; update the client before using this tool", def.Name) }
		return nil
	}
	capability := toolCapability(def.Name)
	if capability == "filesystem" && (def.Name == "sandbox_info" || def.Name == "sandbox_smoke_test") { return nil }
	if capability == "pty" {
		if !capabilityBool(workspace.Capabilities, "shell") || !capabilityBool(workspace.Capabilities, "pty") { return fmt.Errorf("%s is unavailable because this CodeLocal client does not advertise PTY support", def.Name) }
		return nil
	}
	if !capabilityBool(workspace.Capabilities, capability) { return fmt.Errorf("%s is unavailable because this CodeLocal client does not advertise %s support", def.Name, capability) }
	return nil
}
...
func semanticSchema() json.RawMessage { return objectSchema(map[string]any{"path": str("Source file path."), "line": integer("1-based line.", 1, 0), "column": integer("1-based column.", 1, 0), "name": str("Symbol name."), "query": str("Fallback query."), "limit": integer("Maximum results.", 1, 2000), "workspaceKey": workspaceKeySchema}) }
func processSchema(approval map[string]any, withYield bool) json.RawMessage {
	...
	if withYield { p["yieldMs"] = integer("Milliseconds to wait before returning initial output.", 0, 10000) }
	return objectSchema(p, "command")
}
func pollSchema() json.RawMessage { return objectSchema(map[string]any{"processId": str("Process ID."), "stdoutCursor": integer("Stdout byte cursor.", 0, 0), "stderrCursor": integer("Stderr byte cursor.", 0, 0), "workspaceKey": workspaceKeySchema}, "processId") }
func writeProcessSchema() json.RawMessage { return objectSchema(map[string]any{"processId": str("Process ID."), "input": str("Input bytes as UTF-8 text."), "workspaceKey": workspaceKeySchema}, "processId", "input") }
func signalSchema() json.RawMessage { return objectSchema(map[string]any{"processId": str("Process ID."), "signal": map[string]any{"type": "string", "enum": []string{"SIGTERM", "SIGINT", "SIGKILL"}}, "workspaceKey": workspaceKeySchema}, "processId") }

func (s *Service) Handler() http.Handler {
	stream := mcp.NewStreamableHTTPHandler(func(r *http.Request) *mcp.Server { claims, ok := oauth.ClaimsFrom(r.Context()); if !ok || claims.Subject == "" { return nil }; return s.serverFor(claims.Subject) }, &mcp.StreamableHTTPOptions{Stateless: false, JSONResponse: true, MaxRequestBodyBytes: 4 << 20})
	return stream
}

func (s *Service) serverFor(userID string) *mcp.Server {
	s.mu.Lock(); defer s.mu.Unlock()
	if existing := s.servers[userID]; existing != nil { return existing }
	...
	for _, def := range toolDefinitions() {
		...
		server.AddTool(&mcp.Tool{Name: def.Name, Title: title, Annotations: toolAnnotations(def), Description: def.Description, InputSchema: def.Schema}, func(ctx context.Context, req *mcp.CallToolRequest) (*mcp.CallToolResult, error) { return s.callTool(ctx, userID, definition, req) })
	}
	s.servers[userID] = server
	return server
}

func sessionID(req *mcp.CallToolRequest) string { if req != nil && req.Session != nil { return req.Session.ID() }; return "stateless" }
func decodeArgs(req *mcp.CallToolRequest) (map[string]any, error) { args := map[string]any{}; if req == nil || req.Params == nil || len(req.Params.Arguments) == 0 { return args, nil }; if err := json.Unmarshal(req.Params.Arguments, &args); err != nil { return nil, err }; return args, nil }
func textResultWithNotice(value any, isError bool, notice string) *mcp.CallToolResult {
	var text string
	if raw, err := json.MarshalIndent(value, "", "  "); err == nil { text = string(raw) } else { text = fmt.Sprint(value) }
	if strings.TrimSpace(notice) != "" { text = notice + "\n\n" + text }
	return &mcp.CallToolResult{Content: []mcp.Content{&mcp.TextContent{Text: text}}, IsError: isError}
}
func textResult(value any, isError bool) *mcp.CallToolResult { return textResultWithNotice(value, isError, "") }
func errorResult(err error) *mcp.CallToolResult { return textResult(map[string]any{"error": err.Error()}, true) }

func (s *Service) route(userID, session string) string { s.mu.Lock(); defer s.mu.Unlock(); return s.routes[userID][session] }
func (s *Service) setRoute(userID, session, key string) { s.mu.Lock(); defer s.mu.Unlock(); if s.routes[userID] == nil { s.routes[userID] = map[string]string{} }; s.routes[userID][session] = key }

func (s *Service) claimUpdate(userID, session, workspaceKey, installedVersion string) string {
	notice := clientupdate.Evaluate(installedVersion, s.Release); if notice == nil { return "" }
	sessionKey := userID + ":" + session; claimKey := workspaceKey + ":" + notice.Key
	s.mu.Lock(); if s.shownUpdates[sessionKey] == nil { s.shownUpdates[sessionKey] = map[string]struct{}{} }; if _, shown := s.shownUpdates[sessionKey][claimKey]; shown { s.mu.Unlock(); return "" }; s.shownUpdates[sessionKey][claimKey] = struct{}{}; s.mu.Unlock()
	return clientupdate.Render(*notice)
}
func (s *Service) firstUpdateNotice(userID, session string, workspaces []gateway.WorkspaceView) string { for _, workspace := range workspaces { if notice := s.claimUpdate(userID, session, workspace.Key, workspace.ClientVersion); notice != "" { return notice } }; return "" }
...
func (s *Service) callTool(ctx context.Context, userID string, def toolDef, req *mcp.CallToolRequest) (response *mcp.CallToolResult, retErr error) {
	args, err := decodeArgs(req); if err != nil { return errorResult(err), nil }
	session := sessionID(req); _ = s.Store.TouchUserMCPActive(ctx, userID)
	inputBytes, inputTokens := usagecalc.EstimateTokens(args); usageDeviceID := ""; usageWorkspaceID := ""
	defer func() { if response == nil { return }; outputBytes, outputTokens := usagecalc.EstimateTokens(response.Content); event := cloud.MCPUsageEvent{UserID: userID, SessionID: session, DeviceID: usageDeviceID, WorkspaceID: usageWorkspaceID, Tool: def.Name, InputBytes: inputBytes, OutputBytes: outputBytes, InputTokensEst: inputTokens, OutputTokensEst: outputTokens, CreatedAt: time.Now().UnixMilli()}; go func() { usageCtx, cancel := context.WithTimeout(context.Background(), 3*time.Second); defer cancel(); _ = s.Store.RecordMCPUsage(usageCtx, event) }() }()
	if def.Local { return s.callLocal(ctx, userID, session, def.Name, args) }
	explicit, _ := args["workspaceKey"].(string); delete(args, "workspaceKey"); key := strings.TrimSpace(explicit); if key == "" { key = s.route(userID, session) }
	if key == "" {
		catalog, catalogErr := s.Workspaces.Catalog(ctx, userID); if catalogErr != nil { return errorResult(catalogErr), nil }
		active := []gateway.WorkspaceView{}; for _, w := range catalog { if w.Status == "active" { active = append(active, w) } }
		if len(active) == 1 { key = active[0].Key } else if len(active) == 0 { return errorResult(errors.New("no active workspace; call list_workspaces then select_workspace")), nil } else { return errorResult(errors.New("multiple workspaces are active; call list_workspaces then select_workspace, and pass workspaceKey for explicit routing")), nil }
	}
	workspace, err := s.Workspaces.Activate(ctx, userID, key); if err != nil { return errorResult(err), nil }
	usageDeviceID = workspace.DeviceID; usageWorkspaceID = workspace.WorkspaceID
	if err := ensureToolSupported(def, workspace); err != nil { return errorResult(err), nil }
	requestID := cloud.RandomHex(16)
	result, callErr := s.Hub.Call(ctx, userID, key, session, def.Name, args, protocol.SideEffecting(def.Name), requestID); if callErr != nil { return errorResult(callErr), nil }; if !result.OK { return errorResult(errors.New(firstNonEmpty(result.Error, result.ErrorCode, "tool failed"))), nil }
	if terminalExecutionTool(def.Name) { s.Store.Audit(cloud.AuditEvent{UserID: userID, Event: "terminal.executed", DeviceID: workspace.DeviceID, WorkspaceID: workspace.WorkspaceID, Detail: map[string]any{"requestId": requestID, "tool": def.Name}}) }
...
func (s *Service) callLocal(ctx context.Context, userID, session, tool string, args map[string]any) (*mcp.CallToolResult, error) {
	switch tool {
	case "list_devices":
		clients := s.Hub.LocalClients(userID); groups := map[string][]map[string]any{}; for _, c := range clients { groups[c.DeviceID] = append(groups[c.DeviceID], map[string]any{"workspaceId": c.WorkspaceID, "workspaceName": c.WorkspaceName, "key": c.Key, "projectRoot": c.ProjectRoot, "protocolVersion": c.ProtocolVersion, "clientVersion": c.ClientVersion, "capabilities": c.Capabilities, "lastSeenAt": c.LastSeenAt()}) }
		devices := []map[string]any{}; keys := []string{}; for key := range groups { keys = append(keys, key) }; sort.Strings(keys); for _, key := range keys { devices = append(devices, map[string]any{"deviceId": key, "workspaces": groups[key]}) }
		notice := ""; for _, client := range clients { if notice = s.claimUpdate(userID, session, client.Key, client.ClientVersion); notice != "" { break } }; return textResultWithNotice(map[string]any{"devices": devices}, false, notice), nil
	case "list_device_identities":
		devices, err := s.Store.ListDevices(ctx, userID); if err != nil { return errorResult(err), nil }; for i := range devices { devices[i].SecretHash = "" }; return textResult(devices, false), nil
	case "revoke_device":
		id, _ := args["credentialId"].(string); if id == "" { return errorResult(errors.New("credentialId required")), nil }; ok, err := s.Store.RevokeDevice(ctx, userID, id); if err != nil { return errorResult(err), nil }; return textResult(map[string]any{"revoked": ok}, false), nil
	case "rename_device":
		id, _ := args["credentialId"].(string); name, _ := args["deviceName"].(string); ok, err := s.Store.RenameDevice(ctx, userID, id, strings.TrimSpace(name)); if err != nil { return errorResult(err), nil }; return textResult(map[string]any{"renamed": ok}, false), nil
	case "list_workspaces":
		catalog, err := s.Workspaces.Catalog(ctx, userID); if err != nil { return errorResult(err), nil }; notice := s.firstUpdateNotice(userID, session, catalog); return textResultWithNotice(map[string]any{"selectedWorkspace": s.route(userID, session), "workspaces": catalog}, false, notice), nil
	case "select_workspace":
		key, _ := args["key"].(string); workspace, err := s.Workspaces.Activate(ctx, userID, key); if err != nil { return errorResult(err), nil }; s.setRoute(userID, session, key); notice := s.claimUpdate(userID, session, workspace.Key, workspace.ClientVersion); return textResultWithNotice(map[string]any{"selected": key, "workspaceKey": key, "deviceId": workspace.DeviceID, "workspaceId": workspace.WorkspaceID, "workspaceName": workspace.WorkspaceName, "clientVersion": workspace.ClientVersion, "status": "active"}, false, notice), nil
	case "workspace_info":
		key, _ := args["workspaceKey"].(string); if strings.TrimSpace(key) == "" { key = s.route(userID, session) }; if key == "" { return errorResult(errors.New("no workspace selected")), nil }; workspace, err := s.Workspaces.Activate(ctx, userID, key); if err != nil { return errorResult(err), nil }; notice := s.claimUpdate(userID, session, workspace.Key, workspace.ClientVersion); return textResultWithNotice(workspace, false, notice), nil
	default: return errorResult(errors.New("unknown local MCP tool")), nil
	}
}
