...
func TestSubagentExperienceOutcomeRequiresEvidence(t *testing.T) {
	for _, tc := range []struct {
		...
		{status: "done", report: agentTeamReport{Verification: "go test pass"}, outcome: "succeeded", verified: true},
		{status: "blocked", report: agentTeamReport{HaltReason: "approval required"}, outcome: "failed", verified: true},
		{status: "failed", report: agentTeamReport{HaltReason: "tool error"}, outcome: "failed", verified: true},
		{status: "done", report: agentTeamReport{}, outcome: "", verified: false},
		{status: "blocked", report: agentTeamReport{}, outcome: "", verified: false},
	} {
		outcome, verified := subagentExperienceOutcome(tc.status, tc.report)
		if outcome != tc.outcome || verified != tc.verified {
			t.Fatalf("status=%s outcome=%q verified=%v want %q/%v", tc.status, outcome, verified, tc.outcome, tc.verified)
		}
	}
}
