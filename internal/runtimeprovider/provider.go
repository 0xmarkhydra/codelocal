...
func (r *Registry) Register(provider Provider) error {
	if r == nil || provider == nil || strings.TrimSpace(provider.ID()) == "" {
		return ErrInvalidRuntimeProvider
	}
	id := strings.ToLower(strings.TrimSpace(provider.ID()))
	r.mu.Lock()
	defer r.mu.Unlock()
	if _, exists := r.providers[id]; exists {
		return ErrRuntimeProviderExists
	}
...
func Choose(probes []Probe, required Capabilities) (Probe, error) {
	...
	for _, probe := range probes {
		if !probe.Available || !supports(probe.Capabilities, required) {
			continue
		}
		viable = append(viable, probe)
	}
	if len(viable) == 0 {
		return Probe{}, ErrRuntimeProviderMissing
	}
...
func Transition(handle Handle, expectedRevision uint64, next State, at time.Time) (Handle, error) {
	if handle.ID == "" || handle.ProviderID == "" || expectedRevision == 0 || expectedRevision != handle.Revision {
		return Handle{}, ErrRuntimeStateTransition
	}
	if !stateTransitionAllowed(handle.State, next) {
		return Handle{}, ErrRuntimeStateTransition
	}
	if at.IsZero() {
		at = time.Now().UTC()
...
func capabilityCount(c Capabilities) int {
	count := 0
	for _, v := range []bool{c.FileSystem, c.Shell, c.Network, c.Browser, c.Computer, c.Mobile, c.DurableCheckpoint} {
		if v {
			count++
		}
	}
...
func stateTransitionAllowed(from, to State) bool {
	...
	switch from {
	case StateCold:
		return to == StateProvisioning || to == StateFailed
	case StateProvisioning:
		return to == StateActive || to == StateFailed || to == StateStopped
	case StateActive:
		return to == StateIdle || to == StateCheckpointed || to == StateStopped || to == StateFailed
	case StateIdle:
		return to == StateActive || to == StateCheckpointed || to == StateSleeping || to == StateStopped || to == StateFailed
	case StateCheckpointed:
		return to == StateActive || to == StateSleeping || to == StateStopped || to == StateFailed
	case StateSleeping:
		return to == StateActive || to == StateStopped || to == StateFailed
