...
func NewManagedProvider(id string, caps Capabilities, backend ManagedBackend) (*ManagedProvider, error) {
	...
	if id == "" || backend == nil { return nil, ErrInvalidRuntimeProvider }
...
func (p *ManagedProvider) ID() string { if p == nil { return "" }; return p.id }
...
func (p *ManagedProvider) Probe(ctx context.Context) Probe {
	...
	if p == nil || p.backend == nil { probe.Reason = ErrManagedBackendMissing.Error(); return probe }
	available, reason, err := p.backend.Probe(ctx)
	probe.Available = available && err == nil
	probe.Reason = strings.Join(strings.Fields(reason), " ")
	if err != nil && probe.Reason == "" { probe.Reason = "backend_probe_failed" }
	return probe
}
...
func (p *ManagedProvider) Provision(ctx context.Context, req Request) (Handle, error) {
	...
	probe := p.Probe(ctx)
	if !probe.Available { return Handle{}, ErrRuntimeProviderMissing }
	ref, err := p.backend.Provision(ctx, req)
	if err != nil { return Handle{}, err }
	ref = strings.TrimSpace(ref)
	if ref == "" { return Handle{}, ErrManagedBackendRef }
	now := time.Now().UTC()
...
func (p *ManagedProvider) Checkpoint(ctx context.Context, handle Handle) (Handle, error) {
	...
	if err != nil { return Handle{}, err }
	if !p.caps.DurableCheckpoint { return Handle{}, ErrRuntimeStateTransition }
	checkpoint, err := p.backend.Checkpoint(ctx, handle.ID)
	if err != nil { return Handle{}, err }
	checkpoint = strings.TrimSpace(checkpoint)
	if checkpoint == "" { return Handle{}, ErrManagedBackendRef }
	preview.CheckpointRef = checkpoint
	return preview, nil
}

func (p *ManagedProvider) Sleep(ctx context.Context, handle Handle) (Handle, error) {
	if !p.caps.DurableCheckpoint || strings.TrimSpace(handle.CheckpointRef) == "" { return Handle{}, ErrRuntimeStateTransition }
	preview, err := p.preview(handle, StateSleeping)
	if err != nil { return Handle{}, err }
	if err := p.backend.Sleep(ctx, handle.ID); err != nil { return Handle{}, err }
	return preview, nil
}
...
func (p *ManagedProvider) Resume(ctx context.Context, handle Handle) (Handle, error) {
	...
	if err != nil { return Handle{}, err }
	if err := p.backend.Resume(ctx, handle.ID); err != nil { return Handle{}, err }
	return preview, nil
}
...
func (p *ManagedProvider) Stop(ctx context.Context, handle Handle) (Handle, error) {
	...
	if err != nil { return Handle{}, err }
	if err := p.backend.Stop(ctx, handle.ID); err != nil { return Handle{}, err }
	return preview, nil
}
