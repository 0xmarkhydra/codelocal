...
func TestManagedProviderLifecycle(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if handle.State != StateActive || handle.Revision != 1 { t.Fatalf("unexpected provisioned handle: %+v", handle) }
	handle, err = provider.Checkpoint(context.Background(), handle)
	if err != nil { t.Fatal(err) }
	if handle.State != StateCheckpointed || handle.CheckpointRef != "checkpoint-1" { t.Fatalf("unexpected checkpoint: %+v", handle) }
	handle, err = provider.Sleep(context.Background(), handle)
	if err != nil { t.Fatal(err) }
	if handle.State != StateSleeping { t.Fatalf("expected sleeping, got %s", handle.State) }
	handle, err = provider.Resume(context.Background(), handle)
	if err != nil { t.Fatal(err) }
	if handle.State != StateActive { t.Fatalf("expected active, got %s", handle.State) }
	handle, err = provider.Stop(context.Background(), handle)
	if err != nil { t.Fatal(err) }
	if handle.State != StateStopped { t.Fatalf("expected stopped, got %s", handle.State) }
}
...
func TestManagedProviderFailsClosed(t *testing.T) {
	...
	if err == nil { t.Fatal("expected unsupported capability to fail") }
	...
	if _, err := provider.Sleep(context.Background(), handle); err == nil { t.Fatal("sleep without durable checkpoint must fail") }
}
