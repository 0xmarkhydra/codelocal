...
func normalizeExperienceInput(input ExperienceInput) (ExperienceInput, error) {
	...
	rawSubagentID := strings.TrimSpace(input.SubagentID)
	rawSubagentRole := strings.TrimSpace(input.SubagentRole)
	rawSubagentScope := strings.TrimSpace(input.SubagentScope)
	input.SubagentID = normalizeSubagentRoutingID(rawSubagentID)
	input.SubagentRole = normalizeSubagentRole(rawSubagentRole)
	input.SubagentScope = normalizeSubagentScope(rawSubagentScope)
	...
	if rawSubagentID != "" && input.SubagentID == "" {
		return ExperienceInput{}, errors.New("experience subagent id is invalid")
	}
	if rawSubagentRole != "" && input.SubagentRole == "" {
		return ExperienceInput{}, errors.New("experience subagent role is invalid")
	}
	if rawSubagentID == "" && (rawSubagentRole != "" || rawSubagentScope != "" || input.SubagentVerified) {
		return ExperienceInput{}, errors.New("experience subagent metadata requires subagent id")
	}
	if input.SubagentID != "" {
...
func normalizeSubagentRole(value string) string {
	role := normalizeSubagentRoutingID(value)
	switch role {
	case "quick", "investigator", "implementer", "tester", "reviewer", "security", "deep":
		return role
	default:
		return ""
	}
