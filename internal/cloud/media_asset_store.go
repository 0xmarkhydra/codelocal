...
func (s *Store) MediaAssetByOwnerHash(ctx context.Context, ownerUserID, sha256 string) (MediaAsset, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return MediaAsset{}, ErrMediaAssetNotFound
	}
	if err != nil {
...
func (s *Store) loadMediaVariants(ctx context.Context, asset MediaAsset) (MediaAsset, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&variant.AssetID, &variant.Variant, &variant.ObjectKey, &variant.ContentType, &variant.Size, &variant.Width, &variant.Height, &variant.SHA256, &variant.CreatedAt); err != nil {
			return MediaAsset{}, err
		}
...
func (s *Store) CompleteMediaAsset(ctx context.Context, ownerUserID, assetID string, width, height int, variants []MediaVariant) (MediaAsset, error) {
	...
	if asset.OwnerUserID != strings.TrimSpace(ownerUserID) {
		return MediaAsset{}, ErrMediaAssetForbidden
	}
	if width <= 0 || height <= 0 || len(variants) == 0 {
	...
	now := time.Now().UnixMilli()
	if _, err = tx.Exec(ctx, `DELETE FROM codelocal_media_variants WHERE asset_id=$1`, asset.ID); err != nil {
		return MediaAsset{}, err
	}
	for _, variant := range variants {
		if variant.Variant != "original" && variant.Variant != "thumb" && variant.Variant != "medium" && variant.Variant != "large" {
			return MediaAsset{}, ErrMediaAssetInvalid
		}
		if variant.ObjectKey == "" || variant.ContentType == "" || variant.Size <= 0 || variant.Width <= 0 || variant.Height <= 0 || len(variant.SHA256) != 64 {
			return MediaAsset{}, ErrMediaAssetInvalid
		}
		if variant.Variant != "original" && variant.ContentType != "image/webp" {
			return MediaAsset{}, ErrMediaAssetInvalid
		}
		if _, err = tx.Exec(ctx, `INSERT INTO codelocal_media_variants(asset_id,variant,object_key,content_type,size,width,height,sha256,created_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)`, asset.ID, variant.Variant, variant.ObjectKey, variant.ContentType, variant.Size, variant.Width, variant.Height, variant.SHA256, now); err != nil {
			return MediaAsset{}, err
		}
	}
	if _, err = tx.Exec(ctx, `UPDATE codelocal_media_assets SET width=$1,height=$2,status='ready',error_code='',updated_at=$3 WHERE asset_id=$4`, width, height, now, asset.ID); err != nil {
		return MediaAsset{}, err
	}
...
func (s *Store) UpsertMediaUnderstanding(ctx context.Context, ownerUserID string, input MediaUnderstanding) (MediaUnderstanding, error) {
	...
	if err != nil {
		return MediaUnderstanding{}, err
	}
	if asset.OwnerUserID != strings.TrimSpace(ownerUserID) {
		return MediaUnderstanding{}, ErrMediaAssetForbidden
	}
