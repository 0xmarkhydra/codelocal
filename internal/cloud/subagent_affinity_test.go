...
func TestSubagentExperienceRequiresVerifiedOutcome(t *testing.T) {
	_, err := normalizeExperienceInput(ExperienceInput{
		...
		VerificationSummary: "done", Verified: true, SubagentID: "explorer", SubagentVerified: false,
	})
	if err == nil {
		t.Fatal("unverified subagent outcome must not become Experience")
	}
	badScope := ExperienceInput{
		UserID: "user", Objective: "survey auth", Outcome: "succeeded",
		VerificationSummary: "done", Verified: true,
		SubagentID: "explorer", SubagentScope: "wide", SubagentVerified: true,
	}
	if _, err := normalizeExperienceInput(badScope); err == nil {
		t.Fatal("bad subagent scope accepted")
	}
	ok := ExperienceInput{
		UserID: "user", Objective: "survey auth", Outcome: "succeeded",
		VerificationSummary: "done", Verified: true,
	...
	if normalized.SubagentID != "explorer" || normalized.SubagentScope != "project" {
