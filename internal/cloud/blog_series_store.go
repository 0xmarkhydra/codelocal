...
func (s *Store) CreateBlogSeries(ctx context.Context, input BlogSeriesDraft) (BlogSeries, error) {
	...
	if input.CoverAssetID != "" {
		...
		if !ready {
			return BlogSeries{}, ErrBlogInvalid
		}
	}
	...
	if err != nil {
		return BlogSeries{}, mapBlogWriteError(err)
	}
	slots := map[string]string{}
	if input.CoverAssetID != "" {
		slots["cover"] = input.CoverAssetID
	}
	if err := syncMediaAssetRefsTx(ctx, tx, input.AuthorUserID, "blog_series", seriesID, slots); err != nil {
		return BlogSeries{}, err
	}
...
func (s *Store) BlogSeriesByID(ctx context.Context, seriesID string) (BlogSeries, error) {
	...
	if seriesID == "" {
		return BlogSeries{}, ErrBlogNotFound
	}
...
func (s *Store) UpdateBlogSeries(ctx context.Context, actorUserID string, admin bool, seriesID string, input BlogSeriesUpdate) (BlogSeries, error) {
	...
	if current.AuthorUserID != actorUserID && !admin {
		return BlogSeries{}, ErrBlogForbidden
	}
	if input.CoverAssetID != "" {
	...
	if err := syncMediaAssetRefsTx(ctx, tx, current.AuthorUserID, "blog_series", current.ID, slots); err != nil {
		return BlogSeries{}, err
	}
	if err := tx.Commit(ctx); err != nil {
		return BlogSeries{}, err
...
func (s *Store) DeleteBlogSeries(ctx context.Context, actorUserID string, admin bool, seriesID string) error {
	...
	if current.AuthorUserID != strings.TrimSpace(actorUserID) && !admin {
	...
	if _, err = tx.Exec(ctx, `UPDATE codelocal_blog_posts SET series_id=NULL,series_part=NULL,updated_at=$1 WHERE series_id=$2 AND deleted_at=0`, now, current.ID); err != nil {
		return err
	}
	if err := syncMediaAssetRefsTx(ctx, tx, current.AuthorUserID, "blog_series", current.ID, map[string]string{}); err != nil {
		return err
	}
