...
func SelectRuntimeGeneration(taskID string, rollout RuntimeRollout) (RuntimeGeneration, error) {
	...
	if rollout.ForceV1 || !rollout.Enabled { return RuntimeV1, nil }
	if rollout.ForceV2 { return RuntimeV2, nil }
	if rollout.Percent == 0 { return RuntimeV1, nil }
	if rollout.Percent == 100 { return RuntimeV2, nil }
	sum := sha256.Sum256([]byte(taskID))
	bucket := int(binary.BigEndian.Uint32(sum[:4]) % 100)
	if bucket < rollout.Percent { return RuntimeV2, nil }
	return RuntimeV1, nil
}

func PinRuntimeGeneration(bundle Bundle, generation RuntimeGeneration) (Bundle, error) {
	if generation != RuntimeV1 && generation != RuntimeV2 { return Bundle{}, ErrInvalidRuntimeGeneration }
	if bundle.RuntimeGeneration != "" && bundle.RuntimeGeneration != generation { return Bundle{}, ErrInvalidRuntimeGeneration }
