...
func (r *Runtime) ExecuteGuarded(parent context.Context, input Program, guard Guardrails) (Report, error) {
	if r == nil {
		return Report{}, ErrInvalidProgram
	}
	program, err := normalizeProgram(input, r.limits)
	if err != nil {
		return Report{}, err
	}
	if guard.PerStepTimeout <= 0 {
		guard.PerStepTimeout = r.limits.Timeout
	}
	if guard.PerStepTimeout > 5*time.Minute {
		guard.PerStepTimeout = 5 * time.Minute
	}
	if guard.MaxRetries < 0 {
		guard.MaxRetries = 0
	}
	if guard.MaxRetries > 3 {
		guard.MaxRetries = 3
	}
	if guard.MaxReducedTokens <= 0 {
		guard.MaxReducedTokens = r.limits.MaxStepTokens * len(program.Steps)
	}
	if guard.MaxReducedTokens > 64_000 {
		guard.MaxReducedTokens = 64_000
	}
	...
	for _, step := range program.Steps {
		...
		for {
			attempts++
			child := *r
			child.limits = r.limits
			child.limits.Timeout = guard.PerStepTimeout
			child.limits.MaxOperations = 1
			partial, runErr := child.Execute(parent, Program{ID: program.ID + ":" + step.ID, Steps: []Step{step}})
			if runErr == nil {
				if len(partial.Steps) != 1 {
					return report, ErrProgramStep
				}
				report.Steps = append(report.Steps, partial.Steps[0])
				report.OriginalTokens += partial.OriginalTokens
				report.ReducedTokens += partial.ReducedTokens
				if report.ReducedTokens > guard.MaxReducedTokens {
					return report, ErrGuardedTokenBudget
				}
				break
			}
			...
			if attempts > guard.MaxRetries || guard.Retryable == nil || !guard.Retryable(binding.Capability, runErr) {
				return report, runErr
			}
		}
	}
