...
- [x] Apple Developer ID signing/notarization CI path is implemented for packaged native workers: complete Developer ID secrets trigger hardened-runtime timestamped `codesign` + strict verification; complete notarization credentials trigger `notarytool --wait`; partial credentials fail closed, and `CODELOCAL_REQUIRE_APPLE_SIGNING=true` can make unsigned release artifacts impossible. Actual signed/notarized production proof still requires repository Apple credentials and a real release run; source does not claim that proof before it happens.
