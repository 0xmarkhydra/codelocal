...
- [x] Apple Developer ID signing path is wired through a dedicated `codelocal-signing` self-hosted macOS runner: GitHub-hosted runners build the unsigned `arm64` + `amd64` workers, the signing runner uses the Developer ID identity already held in the local Keychain (no `.p12` export or source checkout on the signing job), verifies the signature, and returns signed artifacts to the publish job. Notarization remains gated on the complete Apple ID + Team ID + app-specific-password set and uses `notarytool --wait`; partial auth credentials fail closed. Actual production signing/notarization proof still requires the real release run and an online signing runner.
