...
- [x] Apple Developer ID signing/notarization path is wired through a dedicated `codelocal-signing` self-hosted macOS runner: GitHub-hosted runners build the unsigned `arm64` + `amd64` workers, the signing runner uses the Developer ID identity already held in the local Keychain (no `.p12` export or source checkout on the signing job), verifies the signature, and notarizes with a local `notarytool` Keychain profile before returning signed artifacts to the publish job. Apple ID and the app-specific password stay in the Mac Keychain rather than GitHub Secrets, and a missing notarization profile fails closed. Actual production signing/notarization proof still requires the real release run and an online signing runner.
...
- [x] Credential-input preflight is fail-closed: native AX metadata is resolved before typing approval when available, secure/password/OTP/API-key targets require fresh approval, opaque Browser fill refs require fresh approval, and semantic batches containing typing require one fresh approval because later dynamic fields may not exist during preflight.
- [x] Typed values are not echoed in native/JXA action results, and secure AX fields suppress their values from compatibility tree/element reads as well as the native worker.
- [x] The legacy animated agent-cursor overlay is disabled by default because it launches `osascript`; `CODELOCAL_AGENT_CURSOR=1` is an explicit diagnostic/visual opt-in. The persistent menu-bar Computer Use indicator remains the default transparency surface, keeping decorative process spawning off the native hot path.
- [ ] Remove remaining `osascript` compatibility paths after native packaging reaches production; foreground focus/raw physical keyboard-pointer fallbacks intentionally remain outside the normal background lane, and the legacy cursor overlay may remain opt-in until a persistent-native renderer replaces it.
