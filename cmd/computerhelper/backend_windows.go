...
func windowsInvoke(ctx context.Context, windowID, elementID string) (any, error) {
	if windowID == "" || elementID == "" { return nil, errors.New("windowId and elementId are required for UIA invoke") }
	script := windowsUIAPrelude + `
...
func windowsKey(ctx context.Context, key string) (any, error) {
	...
	if !ok && len([]rune(key)) == 1 {
		...
		if r >= 'A' && r <= 'Z' { vk=uint16(r); ok=true }
		if r >= '0' && r <= '9' { vk=uint16(r); ok=true }
	}
	if !ok { return nil, errors.New("unsupported Windows key") }
...
func windowsDrag(ctx context.Context, x1,y1,x2,y2 float64) (any,error) {
	...
	text,err:=windowsPowerShell(ctx,script,env);if err!=nil{return nil,err};return windowsJSON(text)
}

func platformHandle(ctx context.Context, input request) (any, error) {
	switch input.Operation {
	case "status": return platformCapabilities(), nil
	case "list_windows": return windowsList(ctx)
	case "ui_tree": return windowsUITree(ctx,stringValue(input.Arguments,"windowId"))
	case "screenshot": return windowsScreenshot(ctx,stringValue(input.Arguments,"windowId"))
	case "focus": return windowsFocus(ctx,stringValue(input.Arguments,"windowId"))
	case "click":
		if element:=stringValue(input.Arguments,"elementId");element!="" { return windowsInvoke(ctx,stringValue(input.Arguments,"windowId"),element) }
		x,okX:=numberValue(input.Arguments,"x");y,okY:=numberValue(input.Arguments,"y");if !okX||!okY{return nil,errors.New("computer_click requires elementId+windowId or x/y")};return windowsClick(ctx,x,y)
	case "type": return windowsType(ctx,stringValue(input.Arguments,"text"))
	case "key": return windowsKey(ctx,stringValue(input.Arguments,"key"))
	case "scroll": dx,_:=numberValue(input.Arguments,"deltaX");dy,_:=numberValue(input.Arguments,"deltaY");return windowsScroll(ctx,dx,dy)
	case "drag":
		x1,o1:=numberValue(input.Arguments,"fromX");y1,o2:=numberValue(input.Arguments,"fromY");x2,o3:=numberValue(input.Arguments,"toX");y2,o4:=numberValue(input.Arguments,"toY");if !o1||!o2||!o3||!o4{return nil,errors.New("computer_drag requires fromX/fromY/toX/toY")};return windowsDrag(ctx,x1,y1,x2,y2)
	default: return nil, errors.New("unsupported Windows Computer Use operation")
	}
}
