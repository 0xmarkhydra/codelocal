...
if (!BASE_URL || !MCP_USER_PASSWORD || !MCP_AUTH_SECRET) {
  console.error("Missing PUBLIC_BASE_URL, MCP_USER_PASSWORD or MCP_AUTH_SECRET");
  process.exit(1);
}
...
function verifyToken(token: string, expectedType: TokenPayload["typ"]): TokenPayload {
  ...
  if (payload.typ !== expectedType || payload.exp <= now) throw new Error("Expired or invalid token");
  if (payload.resource !== MCP_RESOURCE) throw new Error("Token is not valid for this resource");
  return payload;
}

function issueTokens(clientId: string, resource: string, scope = SCOPE) {
  const now = Math.floor(Date.now() / 1000);
  const access_token = signToken({ typ: "access", client_id: clientId, resource, scope, iat: now, exp: now + ACCESS_TTL_SECONDS, jti: randomBytes(16).toString("hex") });
  const refresh_token = signToken({ typ: "refresh", client_id: clientId, resource, scope, iat: now, exp: now + REFRESH_TTL_SECONDS, jti: randomBytes(16).toString("hex") });
  return { access_token, refresh_token, token_type: "Bearer", expires_in: ACCESS_TTL_SECONDS, scope };
}
...
function redirectWithError(redirectUri: string, state: string | undefined, error: string) {
...
oauthRouter.post("/register", express.json({ limit: "64kb" }), (req, res) => {
  const redirectUris = Array.isArray(req.body?.redirect_uris) ? req.body.redirect_uris.filter((v: unknown): v is string => typeof v === "string") : [];
  ...
  clients.set(clientId, { clientId, redirectUris, clientName: typeof req.body?.client_name === "string" ? req.body.client_name : undefined });
  res.status(201).json({
    client_id: clientId,
    client_name: typeof req.body?.client_name === "string" ? req.body.client_name : "MCP client",
...
oauthRouter.get("/authorize", (req, res) => {
  ...
  res.type("html").send(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Authorize CodeLocal</title><style>body{font-family:system-ui,-apple-system,sans-serif;background:#f6f6f6;margin:0;display:grid;place-items:center;min-height:100vh}.card{background:white;padding:28px;border-radius:16px;box-shadow:0 8px 30px #0001;width:min(420px,calc(100% - 40px));box-sizing:border-box}h1{font-size:22px;margin:0 0 8px}p{color:#555;line-height:1.45}input[type=password]{width:100%;box-sizing:border-box;padding:12px;border:1px solid #ccc;border-radius:10px;font-size:16px;margin:10px 0 16px}button{width:100%;padding:12px;border:0;border-radius:10px;background:#111;color:#fff;font-size:16px;font-weight:600}</style></head><body><div class="card"><h1>Authorize CodeLocal</h1><p>Allow ChatGPT to access the MCP tools connected to your local coding client.</p><form method="post" action="/authorize">${hiddenInputs}<label>CodeLocal passphrase</label><input type="password" name="password" required autofocus><button type="submit">Authorize ChatGPT</button></form></div></body></html>`);
});
...
oauthRouter.post("/authorize", (req, res) => {
  ...
  if (!client || !client.redirectUris.includes(redirectUri) || !codeChallenge || resource !== MCP_RESOURCE) {
    res.status(400).send("Invalid OAuth authorization request.");
    return;
  }
  if (!equalSecret(password, MCP_USER_PASSWORD)) {
    res.status(401).send("Invalid CodeLocal passphrase. Go back and try again.");
    return;
  }

  const code = randomBytes(32).toString("base64url");
  authCodes.set(code, { clientId, redirectUri, codeChallenge, resource, scope, expiresAt: Date.now() + CODE_TTL_MS });
  const target = new URL(redirectUri);
...
oauthRouter.post("/token", (req, res) => {
  ...
  try {
    if (grantType === "authorization_code") {
      ...
      res.json(issueTokens(clientId, resource, record.scope));
      return;
    }
    ...
    if (grantType === "refresh_token") {
      ...
      res.json(issueTokens(clientId, resource, payload.scope));
      return;
    }
...
export function requireMcpAuth(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    if (!payload.scope.split(/\s+/).includes("mcp:tools")) {
