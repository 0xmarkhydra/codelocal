...
The shipping CodeLocal runtime, CI path and npm release builder are native Go. Files in this `src/` tree are retained only for compatibility/history while retirement evidence is collected.

Rules:

1. Do not add new production runtime behavior here.
2. Do not make root `package.json` build/test/release scripts execute `src/client-v2.ts`, `src/server-saas.ts` or `src/index.ts`.
...
The `src/` compatibility runtime may be physically removed only in a dedicated retirement change after all of these are true:
...
- public docs, support instructions and known integrations have been checked for direct `src/`, `src/index.ts`, `client-v2` or `server-saas` usage;
