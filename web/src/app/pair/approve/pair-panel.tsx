...
export function PairPanel({ pairingId, approved, error }: { pairingId: string; approved: boolean; error: string }) {
  ...
  if (approved) return <div className={styles.actions}><div className={styles.success}>Device approved. Return to your terminal; CodeLocal will claim its credential automatically.</div><Link className={styles.button} href="/dashboard/devices">View devices</Link></div>;
  if (unavailable) return <div className={styles.actions}><div className={styles.alert}>This pairing request is invalid, expired, or temporarily unavailable.</div><Link className={styles.secondaryButton} href="/dashboard/devices">Open devices</Link></div>;
  if (!account || !pairing) return <div className={styles.hint}>Loading pairing request…</div>;

  return <>
    {error ? <div className={styles.alert}>{error}</div> : null}
    <div className={styles.codeBlock}>{pairing.deviceName}<br />Device ID: {pairing.deviceId}</div>
    <form className={styles.form} method="post" action="/pair/approve">
      ...
      <button className={styles.button} type="submit">Approve device</button>
    </form>
    <form className={styles.form} method="post" action="/logout">
      ...
      <button className={styles.secondaryButton} type="submit">Use another account</button>
    </form>
    <div className={styles.hint}>Signed in as {account.email}. Approval expires at {new Date(pairing.expiresAt).toLocaleString()}.</div>
  </>;
}
