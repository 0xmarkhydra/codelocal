...
export default async function ForgotPasswordPage({ searchParams }: Props) {
  ...
  return (
    <AuthShell
      ...
      description="Password reset codes expire quickly and completion revokes existing signed-in sessions. CodeLocal.Cloud never exposes recovery state to advertisers or third parties."
      panelTitle={t(sent ? "Check your email" : "Forgot your password?")}
      panelSubtitle={t(sent ? "If the account exists, reset instructions have been sent." : "Enter the email used for your CodeLocal.Cloud account.")}
    >
      {sent ? (
        <div className={styles.actions}>
          <div className={styles.success}>{t("Open the email from CodeLocal.Cloud. The reset code expires after 10 minutes.")}</div>
